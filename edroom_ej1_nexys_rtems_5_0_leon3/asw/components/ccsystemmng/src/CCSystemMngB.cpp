...
CCSystemMng::EDROOM_CTX_Top_0::EDROOM_CTX_Top_0(CCSystemMng &act,
	 ...
	 CEDROOMPOOLTEDROOMInt8 & EDROOMpPoolTEDROOMInt8 ):
	...
	EDROOMPoolTEDROOMInt8(EDROOMpPoolTEDROOMInt8)
{
}
...
CCSystemMng::EDROOM_CTX_Top_0::EDROOM_CTX_Top_0(EDROOM_CTX_Top_0 &context):
	...
	EDROOMPoolTEDROOMInt8(context.EDROOMPoolTEDROOMInt8 )
{
...

{
  ...
  TEDROOMInt8 * pSLEDStart_Data = EDROOMPoolTEDROOMInt8.AllocData();
...
*pSLEDStart_Data=CLEDPeriod100Ms;
   //Send message 
   LEDMngCtrl.send(SLEDStart,pSLEDStart_Data,&EDROOMPoolTEDROOMInt8); 
}
	...
	//CEDROOMPOOLTEDROOMInt8

CCSystemMng::EDROOM_CTX_Top_0::CEDROOMPOOLTEDROOMInt8::CEDROOMPOOLTEDROOMInt8(
			TEDROOMUInt32 elemCount,TEDROOMInt8* pMem,bool * pMemMarks):
				CEDROOMProtectedMemoryPool(elemCount, pMem, pMemMarks,
					sizeof(TEDROOMInt8))
{
}

TEDROOMInt8 *	CCSystemMng::EDROOM_CTX_Top_0::CEDROOMPOOLTEDROOMInt8::AllocData()
{
	return(TEDROOMInt8*)CEDROOMProtectedMemoryPool::AllocData();
}
...
CCSystemMng::EDROOM_SUB_Top_0::EDROOM_SUB_Top_0 (CCSystemMng&act
	,CEDROOMMemory *pEDROOMMemory):
		EDROOM_CTX_Top_0(act,
			VCommand,
			EDROOMPoolTEDROOMInt8),
		EDROOMStartUp(StartUp , *this),
		VCommand('z'),
		EDROOMPoolTEDROOMInt8(
			10, pEDROOMMemory->poolTEDROOMInt8,
			pEDROOMMemory->poolMarkTEDROOMInt8)
{
	...
	do
	{
		...
		switch(edroomCurrentTrans.localId)
		{
			...
			case (Init):
				...
				break;
			//Next Transition is SubSysReady
			case (SubSysReady):
				//Next State is Ready
	...
	do
	{
		...
		if (edroomCurrentTrans.distanceToContext == 0)
		{
			...
			switch (edroomCurrentTrans.localId)
			{
				...
				case (DisplaysReady):
					//Exit across the exit point SubSysReady
					edroomCurrentTrans.localId= 
						EDROOM_CTX_Top_0::SubSysReady;
		...
		switch(Msg->signal)
		{
			...
			case (SLEDReady): 
				 ...
				 if (*Msg->GetPInterface() == LEDMngCtrl)
				{
					...
					//Next transition is  StartDisplay
					edroomCurrentTrans.localId= StartDisplay;
					edroomCurrentTrans.distanceToContext = 0;
					edroomValidMsg=true;
				 }
